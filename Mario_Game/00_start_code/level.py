...
class Level:
    def __init__(self, grid, switch, asset_dict, audio): # switch :fn
        ...
        self.level_limits = {
            'left' : -WINDOW_WIDTH,
            'right' : sorted(list(grid['terrain'].keys()), key = lambda pos : pos[0])[-1][0] + 500, # rightmost terrain tile + 500
        }
    ...
    def build_level(self, grid, asset_dict, jump_audio):
        for layer_name, layer in grid.items():
            for pos, data in layer.items():
                ...
                match data:
                    case 0 : self.player = Player(pos, asset_dict['player'], self.all_sprites, self.collision_sprites, jump_audio)
                    case 1 : 
                    ...
                    case 5 : Coin('silver', asset_dict['silver'], pos, [self.all_sprites, self.coin_sprites]) # silver
                    case 6 : Coin('diamond', asset_dict['diamond'], pos, [self.all_sprites, self.coin_sprites]) # diamond
        ...
        for sprites in self.shell_sprites:
            sprites.player = self.player # sprites.player is on pygame.sprite.Sprite

    def get_coins(self):
        collied_coins = pygame.sprite.spritecollide(self.player, self.coin_sprites, True) # return sprites list
        for sprite in collied_coins:
