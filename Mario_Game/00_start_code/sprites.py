...
class Tooth(Generic):
    def __init__(self, assets, pos, group, collision_sprites):
        ...
        self.orientation = 'left' if self.direction.x < 0 else self.orientation
        self.pos = vector(self.rect.topleft)
        self.speed = 120
        self.collision_sprites = collision_sprites

        # detroy tooth at the beggining if he is not on a floor
        if not [sprite for sprite in collision_sprites if sprite.rect.collidepoint(self.rect.midbottom + vector(0,10))]:
            self.kill()

    def animate(self, dt):
        current_animation = self.animation_frame[f'run_{self.orientation}']
        self.frame_index += ANIMATION_SPEED * dt
    ...
    def move(self, dt):
        ...
        if self.direction.x < 0: # moving right
            if [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(left_block)] \
            or not [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(left_gap)]:
                self.direction.x *= -1
                self.orientation = 'right'
...
class Shell(Generic):
    ...
    def animate(self, dt):
        current_animation = self.animation_frame[self.status]
        self.frame_index += ANIMATION_SPEED * dt
        ...
        if int(self.frame_index) == 2 and self.status == 'attack' and not self.has_shot:
            pearl_vector = vector(-1,0) if self.orientation == 'left' else vector(1,0)
            offset = (pearl_vector * 50 + vector(0, -10)) if self.orientation == 'left' else (pearl_vector * 20 + vector(0, -10))
            # create a pearl
            Pearl(self.rect.center + offset, pearl_vector, self.pearl_surf, [self.groups()[0] ,self.damage_sprites]) # self.groups : 이 객체가 속한 그룹. 0번은 self.all_sprites그룹임.
            self.has_shot = True
